...
def get_unit(op):
    ...
    if op in ["lwc1", "ld", "lw"]: return "Load"
    return "Int"
...
def simulate_scoreboard(code_text, units, latencies):
    ...
    while True:
        if len(insts) > 0 and all(i.wb > 0 for i in insts): break
        if clock > 1000: break
        ...
        for i in insts:
            ...
            if i.exe > 0 and i.wb == 0:
                war = False
                for j in insts:
                    if j.id < i.id and j.iss > 0 and j.read == 0 and (j.src1 == i.dest or j.src2 == i.dest):
                        war = True
                if war:
                    i.war_stall += 1
            ...
            if i.read > 0 and i.exe == 0:
                ...
                if op_type == "Load": lat = 1 # Match theory
                i.timeline[clock] = "X"
                if clock - i.read == lat:
                    i.exe = clock
                continue
            ...
            if i.iss > 0 and i.read == 0:
                raw = False
                for j in insts:
                    if j.id < i.id and j.dest and (j.dest == i.src1 or j.dest == i.src2):
                        # Can read in the same cycle the producer writes
                        if j.wb == 0 and j.timeline.get(clock) != "W":
                            raw = True
                if raw:
                    i.raw_stall += 1
        ...
        if issue_idx < len(insts):
            i = insts[issue_idx]
            if issue_idx == 0 or (insts[issue_idx-1].iss > 0 and insts[issue_idx-1].iss < clock):
                utype = get_unit(i.op)
                eff_units = units.get(utype, 1)
                if utype == "Load": eff_units = 1
                
                active = sum(1 for j in insts if get_unit(j.op) == utype and j.iss > 0 and j.wb == 0 and j.timeline.get(clock) != "W")
                
                waw = False
                for j in insts:
                    if j.id < i.id and j.iss > 0 and j.wb == 0 and j.dest and j.dest == i.dest:
                        waw = True
...
def simulate_tomasulo(code_text, rs_limits, latencies, cdb_limit=2):
    ...
    while True:
        ...
        for i in insts:
            ...
            # 1.5 Wait for operands (CDB Monitor)
            if i.iss > 0 and i.ex_start == 0:
                raw = False
                for j in insts:
                    if j.id < i.id and j.dest and (j.dest == i.src1 or j.dest == i.src2):
                        # Can start execution in the SAME cycle as WB (bypassing)
                        if j.wb == 0 and j.timeline.get(clock) != "W":
                            raw = True
                if raw:
                    i.raw_stall += 1
