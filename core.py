    ...
    while True:
        ...
        for i in insts:
            if i.read > 0 and i.read < clock and i.exe == 0:
                ...
                # Scoreboard read operand takes 1 cycle, and then execution starts.
                # In standard simulation, exe completes at read + lat
                if clock - i.read == lat:
                    i.exe = clock
        ...
        if issue_idx < len(insts):
            ...
            if issue_idx == 0 or (
                ...
                if utype == "Load":
                    eff_units = 1  # strict matching to PDF for Scoreboard load buffer
                active = sum(
                    1
                    for j in insts
                ...
                if not waw and active < eff_units:
                    i.iss = clock
                    issue_idx += 1
